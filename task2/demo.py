...
for idx in range(len(files)):
    ...
    for k, b in enumerate(bbox):
        ...
        clr2 = colors[np.mod(2, len(colors))]
        print(vert_2D[0, :])
        ...
        e_2d = np.array([[[lmin, lmax], [umin, umin]], [[lmin, lmax], [umax, umax]], [[lmin, lmin], [umin, umax]], [[lmax, lmax], [umin, umax]]])
        for e in e_2d:
            ax1.plot(vert_2D[0, e[0, :]], vert_2D[1, e[1, :]], color=clr2)

        # for e in e_2d:
            # print(vert_2D[0, e], vert_2D[1, e])
            # ax1.plot(vert_2D[0, e], vert_2D[1, e], color=clr2)
